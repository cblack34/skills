...
Active work starts at [`{{RELATIVE_ACTIVE_ENTRY_POINT}}`]({{RELATIVE_ACTIVE_ENTRY_POINT}}). When an archived statement conflicts with the active pack, living reference documents, or current code and tests, the active and current sources win.

<!-- List only the archived records that exist, such as the brief, final acceptance record, milestone-specific design/research, and completed execution plans. Give each a one-line purpose. -->
