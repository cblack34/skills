...
- **Three topologies, one active.** Direct PRs to `main`, a feature spine with leaf PRs, or a dependency-ordered PR stack. The human chooses; the generated workflow keeps exactly one. A stack is human-merged bottom-up, is not a spine, and grants agents no merge authority to `main`; when it is selected, record the merge strategy, stack-advancement method, force-push policy, and who may retarget PRs.
